...
class IndexView(PJAXResponseMixin, ArticleListView):
    '''
    首页
    '''
    # 友情链接类型
...
def fileupload(request):
    ...
    if request.method == 'POST':
        ...
        for filename in request.FILES:
            ...
            url = 'https://resource.zhousc.cn/{type}/{timestr}/{filename}'.format(
...
def test(request):
    ...
    if request.method == 'GET':
        ...
        url = 'www.baidu.com'
        response.append(url)
        return render(request, 'share_layout/test.html')

    else:
        return HttpResponse("only for get")
...
def test0(request):
    ...
    if request.method == 'GET':
        ...
        url = 'www.baidu.com'
        response.append(url)
        return render(request, 'share_layout/test0.html')
